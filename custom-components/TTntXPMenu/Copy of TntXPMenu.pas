{
  Improved to support TntMenuItems by
    M. Mehdipour
  :
    DrawTheText procedure changes: DrawTextEx -> DrawTextExW
    MenueDrawItem procedure changes: FMenuItem: TTntMenuItem
    ToolBarDrawButton procedure: If Buttons is a TTntToolButton it will typecast
      the button as TTntToolButton to draw the TWideCaption.
}
...
uses
  ...
  Menus, TntMenus, Messages, Commctrl, ExtCtrls, TntWindows, TntComCtrls,
  TntExtCtrls;

type
  TTntXPMenu = class(TComponent)
  private
    FActive: boolean;
  ...
  protected
    ...
    procedure DrawTheText(txt, ShortCuttext: WideString; ACanvas: TCanvas;
...
procedure Register;
begin
  RegisterComponents('[Menus]', [TTntXPMenu]);
end;

{ TTntXPMenue }

constructor TTntXPMenu.Create(AOwner: TComponent);
begin
...
procedure TTntXPMenu.ActivateMenuItem(MenuItem: TMenuItem);

  procedure Activate(MenuItem: TMenuItem);
  begin
    if addr(MenuItem.OnDrawItem) <> addr(TTntXPMenu.DrawItem) then
    begin
...
procedure TTntXPMenu.InitMenueItems(Form: TScrollingWinControl; Enable: boolean);
  ...
  begin
    ...
    begin
      ...
      if addr(MenuItem.OnMeasureItem) = addr(TTntXPMenu.MeasureItem) then
...
begin
  ...
  begin
      ...
      begin
        ...
        begin
          if addr(TToolBar(Form.Components[i]).OnCustomDrawButton) =
            addr(TTntXPMenu.ToolBarDrawButton) then
    ...
    if (Form.Components[i] is TControlBar) or
       (Form.Components[i] is TTntControlBar) then
      if not (csDesigning in ComponentState) then
      begin
        if Enable then
        begin
          ...
          else if Form.Components[i] is TTntControlBar then
          begin
            if (not assigned(TTntControlBar(Form.Components[i]).OnBandPaint))
              or (FOverrideOwnerDraw) then
            begin
              TTntControlBar(FForm.Components[i]).OnBandPaint := ControlBarPaint;
            end;
          end;
        end
        else
        begin
          ...
          begin
            if addr(TControlBar(Form.Components[i]).OnBandPaint) =
              addr(TTntXPMenu.ControlBarPaint) then
              TControlBar(Form.Components[i]).OnBandPaint := nil;
          end
          else if Form.Components[i] is TTntControlBar then
          begin
            if addr(TTntControlBar(Form.Components[i]).OnBandPaint) =
              addr(TTntXPMenu.ControlBarPaint) then
              TTntControlBar(Form.Components[i]).OnBandPaint := nil;
          end;
        end;
      end;
...
procedure TTntXPMenu.DrawItem(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
  Selected: Boolean);
begin
  if FActive then
...
function TTntXPMenu.GetImageExtent(MenuItem: TMenuItem): TPoint;
var
...
procedure TTntXPMenu.MeasureItem(Sender: TObject; ACanvas: TCanvas;
  var Width, Height: Integer);
var
  s: WideString;
  W, H: integer;
...
procedure TTntXPMenu.MenueDrawItem(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
  Selected: Boolean);
var
  txt: WideString;
  B: TBitmap;
...
{$IFDEF VER5U}
procedure TTntXPMenu.ToolBarDrawButton(Sender: TToolBar;
  Button: TToolButton; State: TCustomDrawState; var DefaultDraw: Boolean);
...
//-----------
  ...
  begin
    ...
    //Draw TWideCaption
      DrawTheText((Button as TTntToolButton).Caption, '',
        ACanvas, ARect,
...
procedure TTntXPMenu.ControlBarPaint(Sender: TObject; Control: TControl;
  Canvas: TCanvas; var ARect: TRect; var Options: TBandPaintOptions);
var
...
procedure TTntXPMenu.SetGlobalColor(ACanvas: TCanvas);
begin
//-----
...
procedure TTntXPMenu.DrawTopMenuItem(Sender: TObject; ACanvas: TCanvas;
...
procedure TTntXPMenu.DrawCheckedItem(FMenuItem: TMenuItem; Selected,
 HasImgLstBitmap: boolean; ACanvas: TCanvas; CheckedRect: TRect);
var
  X1, X2: integer;
begin
...
procedure TTntXPMenu.DrawTheText(txt, ShortCuttext: WideString; ACanvas: TCanvas; TextRect: TRect;
...
procedure TTntXPMenu.DrawIcon(Sender: TObject; ACanvas: TCanvas; B: TBitmap;
 IconRect: Trect; Hot, Selected, Enabled, Checked, FTopMenu,
 IsRightToLeft: boolean);
var
  DefColor: TColor;
...
procedure TTntXPMenu.DrawArrow(ACanvas: TCanvas; X, Y: integer);
begin
  ACanvas.MoveTo(X, Y);
...
function TTntXPMenu.TopMenuFontColor(ACanvas: TCanvas; Color: TColor): TColor;
var
  r, g, b, avg: integer;
begin
...
procedure TTntXPMenu.SetActive(const Value: boolean);
begin
...
procedure TTntXPMenu.SetAutoDetect(const Value: boolean);
begin
  FAutoDetect := Value;
end;

procedure TTntXPMenu.SetForm(const Value: TScrollingWinControl);
var
  Hold: boolean;
begin
  if Value <> FForm then
...
procedure TTntXPMenu.SetFont(const Value: TFont);
begin
  FFont.Assign(Value);
  Windows.DrawMenuBar(FForm.Handle);
...
procedure TTntXPMenu.SetColor(const Value: TColor);
begin
  FColor := Value;
end;

procedure TTntXPMenu.SetIconBackColor(const Value: TColor);
begin
  FIconBackColor := Value;
end;

procedure TTntXPMenu.SetMenuBarColor(const Value: TColor);
begin
  FMenuBarColor := Value;
  Windows.DrawMenuBar(FForm.Handle);
end;

procedure TTntXPMenu.SetCheckedColor(const Value: TColor);
begin
  FCheckedColor := Value;
end;

procedure TTntXPMenu.SetSeparatorColor(const Value: TColor);
begin
  FSeparatorColor := Value;
end;

procedure TTntXPMenu.SetSelectBorderColor(const Value: TColor);
begin
  FSelectBorderColor := Value;
end;

procedure TTntXPMenu.SetSelectColor(const Value: TColor);
begin
  FSelectColor := Value;
end;

procedure TTntXPMenu.SetDisabledColor(const Value: TColor);
begin
  FDisabledColor := Value;
end;

procedure TTntXPMenu.SetSelectFontColor(const Value: TColor);
begin
  FSelectFontColor := Value;
end;

procedure TTntXPMenu.SetIconWidth(const Value: integer);
begin
  FIconWidth := Value;
end;

procedure TTntXPMenu.SetDrawSelect(const Value: boolean);
begin
  FDrawSelect := Value;
end;
...
procedure TTntXPMenu.SetOverrideOwnerDraw(const Value: boolean);
begin
...
procedure TTntXPMenu.SetUseSystemColors(const Value: boolean);
begin
  FUseSystemColors := Value;
  Windows.DrawMenuBar(FForm.Handle);                        
end;

procedure TTntXPMenu.SetGradient(const Value: boolean);
begin
  FGradient := Value;
end;

procedure TTntXPMenu.SetFlatMenu(const Value: boolean);
begin
  FFlatMenu := Value;
end;
...
procedure TTntXPMenu.DrawGradient(ACanvas: TCanvas; ARect: TRect;
 IsRightToLeft: boolean);
var
  i: integer;
  v: integer;
...
procedure TTntXPMenu.DrawWindowBorder(hWnd: HWND; IsRightToLeft: boolean);
var
  WRect, CRect: TRect;
  dCanvas: TCanvas;
begin
...
procedure TTntXPMenu.Notification(AComponent: TComponent;
