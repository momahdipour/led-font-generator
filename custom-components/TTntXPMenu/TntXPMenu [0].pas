...
uses
  ...
  Menus, TntMenus, Messages, Commctrl, ExtCtrls, TntWindows, TntDialogs;

type
  TTntXPMenu = class(TTntMenuItem)
  private
    FActive: boolean;
  ...
  protected
    ...
    procedure MeasureItemProc(Sender: TObject; ACanvas: TCanvas;
  ...
  begin
    ...
    begin
      ...
      if (not assigned(MenuItem.OnMeasureItem)) or (FOverrideOwnerDraw) then
        MenuItem.OnMeasureItem := MeasureItemProc;
    end
  end;
...
procedure TTntXPMenu.InitMenueItems(Form: TScrollingWinControl; Enable: boolean);
  ...
  begin
    ...
    begin
      ...
      if addr(MenuItem.OnMeasureItem) = addr(TTntXPMenu.MeasureItemProc) then
...
procedure TTntXPMenu.MeasureItemProc(Sender: TObject; ACanvas: TCanvas;
  var Width, Height: Integer);
var
  s: WideString;
  W, H, wTemp, hTemp: integer;
  P: TPoint;
  IsLine: boolean;
begin
  if FActive then
  begin
    ...
    Caption := s;
    Self.MeasureItem(ACanvas, W, hTemp);

    //W := ACanvas.TextWidth(s);
    if pos('&', s) > 0 then
    begin
      Caption := '&&';
      Self.MeasureItem(ACanvas, wTemp, hTemp);
//      W := W - ACanvas.TextWidth('&');
    ...
    begin
      Caption := s;
      Self.MeasureItem(ACanvas, wTemp, H);
...
begin
  ...
  Inc(ARect.Bottom, 1);
  TextRect := ARect;
//  WideShowMessage('1 ' + (FMenuItem as TTntMenuItem).Caption);
  txt := ' ' + FMenuItem.Caption;
//  WideShowMessage('2' + txt);
