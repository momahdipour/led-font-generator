...
uses
  ...
  StdCtrls, Image32, Graphics32, Blend32, AppEvnts, ExtCtrls;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure PaintBoxClearBuffer(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure Button2Click(Sender: TObject);
    procedure Button3Click(Sender: TObject);
    procedure BitmapLayerMouseDown(Sender: TObject;
      Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure BitmapLayerMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
...
begin
  ...
  Bitmap.SetStippleStep(2 / L); {2 = 3 - 1 = Number of colors in a pattern - 1}
  Bitmap.ResetStippleCounter;
  Bitmap.DrawLineFSP(P1.X, P1.Y, P2.X, P2.Y);
end;
...
procedure TForm1.PaintBoxClearBuffer(Sender: TObject);
begin
  // Declare some empty event to prevent PaintBox from cleaning the buffer
end;
...
begin
  ...
  // disable automatic updating of the screen
  // this program uses AppEventsIdle to update the screen
...
procedure TForm1.BitmapLayerMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
var
  R: TRect;
begin
  if not M then Exit;
//  R := BitmapLayer.BoundsRect;
  OffsetRect(R, X - P.X, Y - P.Y);
//  BitmapLayer.BoundsRect := R;
end;
