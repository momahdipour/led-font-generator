...
var
  ...
  MessageCaptions: array[TMsgDlgType] of String;
  MessageDlgButtonCursor: TCursor = crDefault;
  {ForceDlgCaptionLeftToRight:
   ...
   Right-to-Left language (such as Farsi).
  }
  ForceDlgCaptionLeftToRight: Boolean = False;
...
function GetMessageCaption(MsgType: TMsgDlgType): String;
begin
  try
    Result := MessageCaptions[MsgType];
  except
    raise EInvalidArgument.Create('Unexpected MsgType in GetMessageCaption in Dialogs.');
  end;
...
//    if DlgType <> mtCustom then
      Caption := GetMessageCaption(DlgType); //LoadResString(Captions[DlgType]) else
//      Caption := Application.Title;
    ...
    if ForceDlgCaptionLeftToRight then
...
initialization
  ...
  MessageCaptions[mtWarning] := LoadResString(Captions[mtWarning]);
  MessageCaptions[mtError] := LoadResString(Captions[mtError]);
  MessageCaptions[mtInformation] := LoadResString(Captions[mtInformation]);
  MessageCaptions[mtConfirmation] := LoadResString(Captions[mtConfirmation]);
  MessageCaptions[mtCustom] := EmptyStr;  //To be set to title of the application if empty string

  MessageDlgButtonCursor := crDefault;  //Also default value is set directly where this global variable is defined
